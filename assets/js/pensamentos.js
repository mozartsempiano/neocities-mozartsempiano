(function () {
  ...
  async function loadPosts() {
    try {
      const resp = await fetch("/assets/json/pensamentosposts.json", { cache: "no-cache" });
  ...
  function tagLinks(selectedTags) {
    return (tag) => ` <a href="#" class="tag-link${selectedTags.includes(tag) ? " tag-selected" : ""}" data-tag="${tag}">#${tag}</a>`;
  }
  ...
  function parseDateFromString(str) {
    ...
    if (m2) {
      ...
      const monthIdx = months[mon] !== undefined ? months[mon] : months[mon + "_"];
  ...
  function renderPensamentosTable(posts, selectedTags) {
    ...
    return `
      <table class="pensamentos">
        ${posts
          .map(
            (post) => `
            <tr>
              <td class="postlist-data">${post.data}</td>
              <td>
  ...
  async function init() {
    ...
    function render() {
      document.getElementById("pensamentos-list").innerHTML = renderPensamentosTable(filterPosts(), selectedTags);
      // after rendering, append 'novo' icon next to post title when date is recent
      try {
        document.querySelectorAll(".pensamentos tr").forEach((tr) => {
          const dataEl = tr.querySelector(".postlist-data");
          const titleEl = tr.querySelector(".postlist-titulo");
