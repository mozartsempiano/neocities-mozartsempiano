...
`.trim();
  ...
  async function processDuotoneImages() {
    const images = document.querySelectorAll("img.dither.duotone:not([data-duotone-processed])");
  ...
  function run() {
    ...
    function wrapImage(img) {
      ...
      try {
        ...
        if (imgDisplay)
          wrap.style.display = imgDisplay === "block" ? "block" : imgDisplay === "inline" ? "inline" : "inline-block";
      } catch (e) {}
    ...
    Array.from(document.querySelectorAll("img.dither:not(.duotone)")).forEach(wrapImage);
      ...
      for (const r of records) {
        if (r.type === "childList") {
          r.addedNodes.forEach((n) => {
            if (n.nodeType === 1) {
              if (n.matches && n.matches("img.dither:not(.duotone)")) wrapImage(n);
              if (n.querySelectorAll) n.querySelectorAll("img.dither:not(.duotone)").forEach(wrapImage);

              if (n.matches && n.matches("img.dither.duotone")) hasDuotoneChanges = true;
              if (n.querySelectorAll && n.querySelectorAll("img.dither.duotone").length > 0) hasDuotoneChanges = true;
            }
          });
        } else if (r.type === "attributes" && r.attributeName === "class" && r.target.matches) {
